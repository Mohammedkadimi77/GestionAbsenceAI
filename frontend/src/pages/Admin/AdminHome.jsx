        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-8">
          <div>
            <h1 className="text-2xl font-bold text-slate-800 tracking-tight">
              Dashboard <span className="text-indigo-600">Administrateur</span>
            </h1>
          ...
          <div className="animate-in fade-in duration-300">
            ...
            {tab === "import" && (
              <div className="max-w-lg mx-auto bg-white border border-slate-200 rounded-2xl p-8 shadow-sm">
                ...
                <form onSubmit={onImport} className="space-y-6">
                  <div className="border-2 border-dashed border-slate-100 rounded-xl p-8 text-center bg-slate-50/50 hover:bg-indigo-50/30 hover:border-indigo-200 transition-all cursor-pointer relative group">
                  ...
                  <button type="submit" disabled={!excel || importing} className="w-full py-3 bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all disabled:opacity-50 shadow-md shadow-indigo-100">
                          ...
                          <tr key={j.id} className="hover:bg-slate-50/30 transition-colors">
                            ...
                            <td className="px-6 py-5">
                              <div className="flex flex-col gap-1">
                                ...
                                {j.fileUrl && (
                                  <a href={j.fileUrl} target="_blank" rel="noreferrer" className="text-[10px] text-indigo-500 font-bold hover:underline uppercase tracking-tight">Voir document</a>
                                )}
                              </div>
                            </td>
            ...
            {tab === "alerts" && (
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {alerts.map((a) => (
                  <div key={a.id} className="bg-white border border-slate-200 rounded-2xl p-6 shadow-sm hover:border-indigo-300 transition-all">
                    <div className="flex justify-between items-start mb-4">
                      ...
                      <div className="text-right">
                        ...
                        <span className={`px-2 py-0.5 rounded-lg text-xs font-bold ${a.scoreAnomalie > 60 ? 'bg-rose-50 text-rose-600' : 'bg-indigo-50 text-indigo-600'}`}>{a.scoreAnomalie}%</span>
                      </div>
                    </div>
...
function TabBtn({ active, onClick, label, badge }) {
  return (
    ...
    >
      {label}
      {badge > 0 && (
        <span className="inline-flex items-center justify-center w-5 h-5 bg-indigo-600 text-white text-[9px] rounded-full">
          {badge}
        </span>
      )}
    </button>
